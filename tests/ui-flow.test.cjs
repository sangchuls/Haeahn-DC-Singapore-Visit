...
test('failed model request restores controls and offers local questions',async()=>{
  ...
  await ui.vault.save('test-key-not-real-123','test-password');
...
test('personal keys are encrypted, locked after reload and isolated by person/provider',async()=>{
  const ui=setup();ui.vault.select('임상철','gemini');
  await ui.vault.save('dummy-gemini-secret-123','private-pass-123');
  assert.doesNotMatch(JSON.stringify([...ui.storage]),/dummy-gemini-secret|private-pass/);
  const fresh=setup({},ui.storage);assert.equal(fresh.vault.connection(),null);assert.equal(fresh.vault.saved(),true);
  await assert.rejects(()=>fresh.vault.unlock('wrong-password'),/비밀번호/);
  await fresh.vault.unlock('private-pass-123');assert.equal(fresh.vault.connection().key,'dummy-gemini-secret-123');
  fresh.vault.select('정재수','gemini');assert.equal(fresh.vault.saved(),false);assert.equal(fresh.vault.connection(),null);
  fresh.vault.select('임상철','openai');assert.equal(fresh.vault.saved(),false);
  fresh.vault.select('임상철','gemini');fresh.vault.remove();assert.equal(fresh.vault.saved(),false);assert.equal(fresh.vault.connection(),null);
});
test('missing key opens settings instead of making a doomed API request',async()=>{
  ...
  assert.match(ui.get('fa-answer').textContent,/개인 API 키/);assert.equal(ui.get('fa-ai-settings').open,true);
});
test('specific date questions work online without any key and headers never enter history',async()=>{
  ...
  await ui.vault.save('dummy-private-key-value','private-pass-123');let request;
